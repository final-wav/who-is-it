...
export const QuestionDialog: React.FC<QuestionDialogProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-3 animate-in fade-in duration-100">
      <div className="w-full max-w-lg bg-white border-4 border-amber-400 rounded-3xl p-5 shadow-2xl">
        <div className="flex items-center justify-between pb-2 mb-3 border-b border-stone-200">
          <span className="text-sm font-display font-black text-slate-900 uppercase tracking-wider">
        ...
        {!showCustom ? (
          <div className="flex flex-col gap-1.5 max-h-[60vh] overflow-y-auto">
            {commonQuestions.map((q, idx) => (
              <button
                ...
                className="btn-board text-left px-3.5 py-2.5 rounded-xl bg-stone-100 hover:bg-amber-400 hover:text-slate-950 text-xs sm:text-sm font-bold text-slate-800 border border-stone-200 transition shadow-2xs"
              >
                {q.text}
              </button>
            ))}
          ...
          <form onSubmit={handleCustomSubmit} className="flex gap-2 pt-2">
            <input
              ...
              className="flex-1 px-3.5 py-2.5 bg-stone-50 border border-stone-300 rounded-xl text-xs sm:text-sm text-slate-900 font-bold focus:outline-none focus:border-blue-500"
              autoFocus
            />
            <button
              type="submit"
