...
export const Lobby: React.FC<LobbyProps> = ({
  ...
  return (
    <div className="w-full max-w-xl mx-auto flex flex-col gap-4 p-3 sm:p-5 font-display">
      {/* Header */}
      <div className="relative bg-white dark:bg-zinc-900 border-2 border-stone-200 dark:border-zinc-800 rounded-3xl p-5 sm:p-6 text-center shadow-xl overflow-hidden">
        ...
        <div className="mt-4 flex items-center justify-between gap-3 bg-stone-50 dark:bg-zinc-800/80 p-3 rounded-2xl border border-stone-200 dark:border-zinc-700 max-w-sm mx-auto shadow-inner">
          <div className="flex items-center gap-2">
            <span className="text-xs font-black text-stone-500 dark:text-zinc-400 uppercase">Code:</span>
            <span className="text-2xl font-black text-amber-500 tracking-widest font-mono">{roomId}</span>
          </div>
          <button
            onClick={handleCopyLink}
            className="btn-board px-3 py-1.5 bg-white dark:bg-zinc-700 hover:bg-stone-100 dark:hover:bg-zinc-600 text-slate-900 dark:text-white text-xs font-black rounded-xl flex items-center gap-1.5 border border-stone-300 dark:border-zinc-600 shadow-2xs transition"
          >
        ...
        <div
          className={`rounded-2xl p-4 transition flex items-center justify-between border-2 ${
            p2Ready
              ...
              : 'bg-stone-100 dark:bg-zinc-900/60 border-stone-300 dark:border-zinc-800 text-stone-500 dark:text-zinc-400 border-dashed'
          }`}
        >
          <div>
            <span
            ...
            <span
              className={`text-lg font-black truncate block max-w-[150px] ${
                p2Ready ? 'text-white' : 'text-slate-900 dark:text-zinc-300'
              }`}
            >
              {p2Name}
            </span>
          ...
          <span
            className={`px-3 py-1 rounded-full text-xs font-black ${
              p2Ready
                ...
                : 'bg-white dark:bg-zinc-800 text-stone-500 dark:text-zinc-400 border border-stone-200 dark:border-zinc-700'
            }`}
          >
      ...
      {/* Deck Selector */}
      <div className="bg-white dark:bg-zinc-900 border-2 border-stone-200 dark:border-zinc-800 rounded-2xl p-4 shadow-sm">
        <span className="text-xs font-black text-stone-600 dark:text-zinc-400 uppercase tracking-wider block mb-2 font-sans">
          Kartenpaket wählen
        </span>
      ...
      <div className="flex flex-col gap-2">
        ...
        {onLeave && (
          <button
            onClick={onLeave}
            className="btn-board w-full py-2 bg-stone-100 dark:bg-zinc-800 hover:bg-stone-200 dark:hover:bg-zinc-700 text-stone-600 dark:text-zinc-300 font-bold text-xs uppercase tracking-wider rounded-xl transition border border-stone-300 dark:border-zinc-700 text-center"
          >
            Raum verlassen
          </button>
        )}
